...
xhr.onload = function () {              // When response has loaded
    ...
    if (xhr.status === 200) {             // If server status was ok
        ...
        var htmlContent = '';
        for (var i = 0; i < response.inventory.item.length; i++) {
            var product = new Product(
                response.inventory.item[i].name,
                parseInt(response.inventory.item[i].numInStock),
                parseInt(response.inventory.item[i].price)
            )
            //console.log(parseInt(response.inventory.item[i].numInStock));
    ...
    xhr.open('GET', 'data/stock.json', true);           // Prepare the request
